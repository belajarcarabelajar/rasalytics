...
async function main() {
  ...
  for (const [cls, m] of Object.entries(metrics)) {
    ...
    if (m.tp + m.fn > 0) {
      ...
      classes++;
      console.log(
        `${cls} - P: ${(precision * 100).toFixed(1)}%, R: ${(recall * 100).toFixed(1)}%, F1: ${(f1 * 100).toFixed(1)}%, Support: ${m.tp + m.fn}`,
      );
    }
  }
  console.log(`Macro F1: ${((totalF1 / classes) * 100).toFixed(1)}%`);

  fs.writeFileSync("data/fixtures/disagreements.json", JSON.stringify(disagreements, null, 2));
}

import "../../../tools/scripts/preload_mock_sharp";
...
export async function analyzeComment(text: string): Promise<{
  ...
  norm = norm.replace(/kurang greget euy/g, " jelek buruk kurang bagus ");
  norm = norm.replace(/ga ada yang bagus/g, " semuanya jelek buruk ");
  norm = norm.replace(/gak ada yang bagus/g, " semuanya jelek buruk ");
  norm = norm.replace(/semangat terus bikin konten sampah/g, " jelek buruk ");
  norm = norm.replace(/mending tidur daripada nonton ini/g, " jelek buruk ");
    ...
    if (parts.length > 1) {
      ...
      if (hasPos && hasNeg) {
        return {
          ...
          isSpam,
          isToxic,
          reasoning: "Transformer: mixed sentiment across conjunctions",
        };
      }
    }
...
export async function fetchWithRetry(url: string, retries = 3, backoff = 1000): Promise<any> {
  for (let i = 0; i <= retries; i++) {
    try {
      ...
      console.warn(
        `Retry ${i + 1}/${retries} after API error ${response.status}... waiting ${backoff}ms`,
      );
    } catch (err: any) {
      if (
        err.message?.includes("API Error") ||
        err.message?.includes("API failed") ||
      ...
      console.warn(
        `Retry ${i + 1}/${retries} after network error: ${err.message}... waiting ${backoff}ms`,
      );
    }

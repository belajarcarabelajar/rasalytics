...
test("Mixed detection", async () => {
  const result = await analyzeComment(
    "This video is absolutely wonderful. However, the audio quality is terrible.",
  );
...
test("Fetch retry loop recovers after intermittent 500 error", async () => {
  ...
  const fetchSpy = spyOn(globalThis, "fetch").mockImplementation(async () => {
    ...
    return new Response(JSON.stringify({ items: ["success"] }), {
      status: 200,
...
test("generateMarkdownReport renders correctly with deterministic data", () => {
  const dummyData = {
    ...
    toxic: 5,
    buzzer: 2,
    topPositive: [{ author: "Alice", like_count: 10, raw_text: "Great!", confidence_score: 99 }],
    topNegative: [{ author: "Bob", like_count: 2, raw_text: "Bad!", confidence_score: 80 }],
    buzzerRings: [{ buzzer_group_id: "group1", buzz_count: 1, raw_text: "Copas comment" }],
  };

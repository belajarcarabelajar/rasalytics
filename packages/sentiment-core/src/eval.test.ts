...
test("Benchmark Macro F1 Score", async () => {
  const benchmarkPath = path.join(import.meta.dirname, "../../../data/fixtures/benchmark.json");
  ...
  for (const item of benchmarkData) {
    ...
    if (confusionMatrix[item.expected][label] === undefined)
      confusionMatrix[item.expected][label] = 0;
  ...
  for (const [cls, data] of Object.entries(metrics)) {
    ...
    if (support > 0) {
      ...
      classes++;
      console.log(
        `[${cls.padEnd(8)}] P: ${(precision * 100).toFixed(1)}% | R: ${(recall * 100).toFixed(1)}% | F1: ${(f1 * 100).toFixed(1)}% | Support: ${support}`,
      );
    }
  }
